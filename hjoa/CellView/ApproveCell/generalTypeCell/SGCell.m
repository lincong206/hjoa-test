...
- (void)creatTextViewWithModel:(SGModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        ...
        case 219:           //  合同金额小写（元）
            ...
            break;
        case 220:           //  是否含清包工
            if (model.bpcClearcontractor.integerValue == 0) {
                label.text = @"请选择";
            }else if (model.bpcClearcontractor.integerValue == 1) {
                label.text = @"是";
            }else {
                label.text = @"否";
            }
            break;
        case 221:           //  项目负责人
            if (model.bpcOwnerpromaterial.integerValue == 0) {
                label.text = @"请选择";
            }else if (model.bpcOwnerpromaterial.integerValue == 1) {
                label.text = @"是";
            }else {
                label.text = @"否";
            }
        ...
        case 240:           //  是否需要单项工程责任书
            ...
            break;
        case 241:           //  购买保险方式
            if (model.bpcInsuranceway.integerValue == 0) {
                label.text = @"不限";
            }else if (model.bpcInsuranceway.integerValue == 1) {
                label.text = @"自行购买";
            }else if (model.bpcInsuranceway.integerValue == 2) {
                label.text = @"委托公司购买";
            }else if (model.bpcInsuranceway.integerValue == 3) {
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromSGCell:_noteHeight + 10];
    }
}
