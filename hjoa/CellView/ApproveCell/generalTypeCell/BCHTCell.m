...
- (void)creatTextViewWithModel:(BCHTModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        ...
        case 215:           //  备注
            ...
            break;
        case 216:           //  合同编号
            label.text = model.pcsRealcontractid;
            break;
        case 217:           //  合同名称
            label.text = model.pcsName;
            break;
        case 218:           //  合同份数
            label.text = [NSString stringWithFormat:@"%@",model.pcsCopies];
            break;
        case 219:           //  合同金额小写（元）
            label.text = [NSString stringWithFormat:@"%@",model.pcsProjectprice];
            break;
        case 220:           //  是否含清包工
            if (model.pcsClearcontractor.integerValue == 0) {
                label.text = @"请选择";
            }else if (model.pcsClearcontractor.integerValue == 1) {
                label.text = @"是";
            }else {
                label.text = @"否";
            }
            break;
        case 221:           //  项目负责人
            if (model.pcsOwnerpromaterial.integerValue == 0) {
                label.text = @"请选择";
            }else if (model.pcsOwnerpromaterial.integerValue == 1) {
                label.text = @"是";
            }else {
                label.text = @"否";
            }
            break;
        case 222:           //  合同金额大写
            label.text = model.pcsProjectbigprice;
            break;
        case 223:           //  合同签订日期
            label.text = model.pcsSigneddate;
            break;
        case 224:           //  合同开工日期
            label.text = model.pcsStartdate;
            break;
        case 225:           //  合同竣工日期
            label.text = model.pcsWorkeddate;
            break;
        case 226:           //  工期
            label.text = model.pcsWorkingtime;
            break;
        case 227:           //  合同质保期
            label.text = model.pcsRetentiondate;
            break;
        case 228:           //  合同预付款比例（%）
            label.text = [NSString stringWithFormat:@"%@",model.pcsAdvancepay];
            break;
        case 229:           //  合同预付款金额（元）
            label.text = [NSString stringWithFormat:@"%@",model.pcsAdvancepaym];
            break;
        case 230:           //  合同进度款比例（%）
            label.text = [NSString stringWithFormat:@"%@",model.pcsProgresspay];
            break;
        case 231:           //  合同进度款金额（元）
            label.text = [NSString stringWithFormat:@"%@",model.pcsProgresspaym];
            break;
        case 232:           //  合同竣工款比例（%）
            label.text = [NSString stringWithFormat:@"%@",model.pcsWorkedpay];
            break;
        case 233:           //  合同竣工款金额（元）
            label.text = [NSString stringWithFormat:@"%@",model.pcsWorkedpaym];
            break;
        case 234:           //  合同结算款比例（%）
            label.text = [NSString stringWithFormat:@"%@",model.pcsSettlepay];
            break;
        case 235:           //  合同结算款金额（元）
            label.text = [NSString stringWithFormat:@"%@",model.pcsSettlepaym];
            break;
        case 236:           //  合同质保金比例（%）
            label.text = [NSString stringWithFormat:@"%@",model.pcsRetentionpay];
            break;
        case 237:           //  合同质保金款金额（元）
            label.text = [NSString stringWithFormat:@"%@",model.pcsRetentionpaym];
            break;
        case 238:           //  付款方式
            label.text = model.pcsPayway;
            break;
        case 239:           //  工程责任书状态
            if (model.pcsProjectitemsstate.integerValue == 0) {
                label.text = @"不限";
            }else if (model.pcsProjectitemsstate.integerValue == 1) {
                label.text = @"未返回";
            }else {
                label.text = @"已移交";
            }
        ...
        case 240:           //  是否需要单项工程责任书
            if (model.pcsIssingleitems.integerValue == 0) {
                label.text = @"是";
            }else if (model.pcsIssingleitems.integerValue == 1) {
                label.text = @"否";
            }else {
                label.text = @"";
            }
            break;
        case 241:           //  购买保险方式
            if (model.pcsInsuranceway.integerValue == 0) {
                label.text = @"不限";
            }else if (model.pcsInsuranceway.integerValue == 1) {
                label.text = @"自行购买";
            }else if (model.pcsInsuranceway.integerValue == 2) {
                label.text = @"委托公司购买";
            }else if (model.pcsInsuranceway.integerValue == 3) {
            ...
            break;
        case 242:           //  合同账号章
            if (model.pcsContractaccount.integerValue == 0) {
                label.text = @"华夏银行(陕西省)";
            }else if (model.pcsContractaccount.integerValue == 1) {
                label.text = @"建设银行(江苏省)";
            }else if (model.pcsContractaccount.integerValue == 2) {
                label.text = @"中国银行(其他地区)";
            }else if (model.pcsContractaccount.integerValue == 3) {
                label.text = @"招商银行";
            }else {
                label.text = @"无(默认)";
            }
            break;
        case 243:           //  合同承包范围
            label.text = model.pcsContractingscope;
            break;
        case 244:           //  工程竣工验收条款
            label.text = model.pcsWorkedacceptterm;
            break;
        case 245:           //  合同质保金条款
            label.text = model.pcsRetentionterm;
            break;
        case 246:           //  资料员姓名
            label.text = model.pcsOperatename;
            break;
        case 247:           //  资料员电话
            label.text = model.pcsOperatephone;
            break;
        case 248:           //  资料移交
            _name = @[@"招标文件",@"投标文件",@"中标通知书",@"预算",@"图纸",@"结算通知书",@"验收报告",@"开工报告",@"工程量清单"];
            for (NSString *string in [model.pcsOperatemove componentsSeparatedByString:@","]) {
            ...
            break;
        case 249:           //  合同归档日期
            label.text = model.pcsArchivedate;
            break;
        case 250:           //  归档编号
            label.text = model.pcsArchiveid;
            break;
        case 251:           //  归档人
            label.text = model.pcsArchivename;
            break;
        case 252:           //  备注
            label.text = model.pcsThenote;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromBCHTCell:_noteHeight + 10];
    }
}
