...
@end
@implementation NKDJCell

- (void)creatNKDJApproveUIWithModel:(NKDJModel *)model
{
    _titleArr = @[@"申请人",@"申请日期",@"借款项目",@"垫付项目",@"项目负责人",
                  @"借款金额(元)",@"备注"];
...
- (void)creatTextViewWithModel:(NKDJModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:
            ...
            break;
        case 201:
            label.text = model.emCreatetime;
            break;
        case 202:
            label.text = model.emBorrowproject;
            break;
        case 203:
            label.text = model.emAdvanceproject;
            break;
        case 204:
            label.text = model.uiManagername;
            break;
        case 205:
            label.text = [NSString stringWithFormat:@"%@",model.emMoney];
            break;
        case 206:
            label.text = model.emRemark;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromNKDJCell:_noteHeight + 10];
    }
}
