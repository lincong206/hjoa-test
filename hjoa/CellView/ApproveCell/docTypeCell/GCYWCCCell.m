...
@end
@implementation GCYWCCCell

- (void)creatGCYWCCApproveUIWithModel:(GCYWCCModel *)model
{
    _titleArr = @[@"申请类型",@"编号",@"发起人姓名",@"发起部门",@"出差人",
                  @"出差部门",@"职务",@"出差时间",@"出差地址",@"出差天数",
                  @"项目名称",@"项目内容",@"项目负责人",@"负责人电话",@"应收费用(元)",
                  @"行政人力中心意见",@"委托人意见",@"建议",@"备注"];
...
- (void)creatTextViewWithModel:(GCYWCCModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:           //  发起人姓名
            ...
            break;
        case 201:           //  发起部门
            label.text = model.dpbeDept;
            break;
        case 202:           //  出差人
            label.text = model.dpbeEvectionname;
            break;
        case 203:           //  出差部门
            label.text = model.dpbeEvectiondept;
            break;
        case 204:           //  职务
            label.text = model.dpbeJob;
            break;
        case 205:           //  出差时间
            label.text = [model.dpbeEvectiontime componentsSeparatedByString:@" "].firstObject;
            break;
        case 206:           //  出差地址
            label.text = model.dpbeEvectionassress;
            break;
        case 207:           //  天数
            label.text = model.dpbeDay;
            break;
        case 208:           //  工程名称
            label.text = model.dpbeProjectname;
            break;
        case 209:           //  项目内容
            label.text = model.dpbeProjectcontent;
            break;
        case 210:           //  项目负责人
            label.text = model.dpbeProjectprincipal;
            break;
        case 211:           //  负责人电话
            label.text = model.dpbeProjectmobile;
            break;
        case 212:           //  应收费用
            label.text = [NSString stringWithFormat:@"%@",model.dpbeReceivecost];
            break;
        case 213:           //  委托人意见
            if (model.dpbeEntrustopinion.integerValue == 0) {
                label.text = @"满意";
            }else if (model.dpbeEntrustopinion.integerValue == 1) {
                label.text = @"一般";
            }else {
                label.text = @"不满";
            }
            break;
        case 214:           //  建议
            label.text = model.dpbeSuggest;
            break;
        case 215:           //  备注
            label.text = model.dpbeRemark;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromGCYWCCCell:_noteHeight + 10];
    }
}
@end
