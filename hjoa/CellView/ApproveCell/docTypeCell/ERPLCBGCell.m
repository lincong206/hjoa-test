...
@end
@implementation ERPLCBGCell

- (void)creatERPLCBGApproveUIWithModel:(ERPLCBGModel *)model
{
    _titleArr = @[@"发起人姓名",@"发起部门",@"制单日期",@"申请理由",@"需变更流程名称",
                  @"新流程描述"];
...
- (void)creatTextViewWithModel:(ERPLCBGModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:           //  发起人姓名
            ...
            break;
        case 201:           //  发起部门
            label.text = model.defcaDept;
            break;
        case 202:           //  制单日期
            label.text = [model.defcaCreatetime componentsSeparatedByString:@"."].firstObject;
            break;
        case 203:           //  申请理由
            label.text = model.defcaReason;
            break;
        case 204:           //  需变更流程名称
            label.text = model.defcaName;
            break;
        case 205:           //  新流程描述
            label.text = model.defcaNewflow;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromERPLCBGCell:_noteHeight + 10];
    }
}
