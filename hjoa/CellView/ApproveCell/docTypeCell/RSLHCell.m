...
@implementation RSLHCell

- (void)creatRSLHApproveUIWithModel:(RSLHModel *)model
{
    _titleArr = @[@"发起人姓名",@"发起部门",@"制单日期",@"出生年月",@"籍贯",
                  @"职务",@"文化程度",@"职称",@"入职时间",@"申请理由"];
...
- (void)creatTextViewWithModel:(RSLHModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:           //  发起人姓名
            ...
            break;
        case 201:           //  发起部门
            label.text = model.dpsaDept;
            break;
        case 202:           //  制单日期
            label.text = [model.dpsaCreatetime componentsSeparatedByString:@"."].firstObject;
            break;
        case 203:           //  出生年月
            label.text = [model.dpsaDirthday componentsSeparatedByString:@" "].firstObject;
            break;
        case 204:           //  籍贯
            label.text = model.dpsaNativeplace;
            break;
        case 205:           //  职务
            label.text = model.dpsaJob;
            break;
        case 206:           //  文化程度
            label.text = model.dpsaCulture;
            break;
        case 207:           //  职称
            label.text = model.dpsaJobtitle;
            break;
        case 208:           //  入职时间
            label.text = [model.dpsaEmploytime componentsSeparatedByString:@" "].firstObject;
            break;
        case 209:           //  申请理由
            label.text = model.dpsaReason;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromRSLHCell:_noteHeight + 10];
    }
}
