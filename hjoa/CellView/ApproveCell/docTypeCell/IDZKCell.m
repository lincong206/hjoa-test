...
@end
@implementation IDZKCell

- (void)creatIDZKApproveUIWithModel:(IDZKModel *)model
{
    _titleArr = @[@"发起人姓名",@"发起部门",@"制单日期"];
...
- (void)creatTextViewWithModel:(IDZKModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:           //  发起人姓名
            ...
            break;
        case 201:           //  发起部门
            label.text = model.diceDept;
            break;
        case 202:           //  制单日期
            label.text = [model.diceCreatetime componentsSeparatedByString:@"."].firstObject;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromIDZKCell:_noteHeight + 10];
    }
}
