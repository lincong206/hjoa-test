...
@implementation YZHSCell

- (void)creatYZHSApproveUIWithModel:(YZHSModel *)model
{
    _titleArr = @[@"发起人姓名",@"发起部门",@"发起时间",@"原领用人",@"退章日期",
                  @"领用名称",@"项目名称",@"经办人",@"图章存样",@"备注"];
...
- (void)creatTextViewWithModel:(YZHSModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:           //  发起人姓名
            ...
            break;
        case 201:           //  发起部门
            label.text = model.dsrDept;
            break;
        case 202:           //  发起时间
            _date = [NSDate dateWithTimeIntervalSince1970:[model.dsrDate doubleValue] / 1000.0];
            ...
            break;
        case 203:           // 原领用人
            label.text = model.dsrName;
            break;
        case 204:           //  退章日期
            label.text = [model.dsrRecycletime componentsSeparatedByString:@" "].firstObject;
            break;
        case 205:           //  领用名称
            label.text = model.dsrSealname;
            break;
        case 206:           //  项目名称
            label.text = model.dsrProjectname;
            break;
        case 207:           //  经办人
            label.text = model.dsrRemark;
            break;
        case 208:           //  图章存样
            label.text = model.dsrResponsible;
            break;
        case 209:           //  备注
            label.text = model.dsrSealbackups;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromYZHSCell:_noteHeight + 10];
    }
}
