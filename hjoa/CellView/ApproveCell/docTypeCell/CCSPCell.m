...
@end
@implementation CCSPCell

- (void)creatCCSPApproveUIWithModel:(CCSPModel *)model
{
    _titleArr = @[@"发起人姓名",@"发起部门",@"制单日期",@"出差人",@"部门",
                  @"职务",@"出差开始时间",@"出差结束时间",@"出差天数",@"出差地点",
                  @"出差事由"];
...
- (void)creatTextViewWithModel:(CCSPModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:           //  发起人姓名
            ...
            break;
        case 201:           //  发起部门
            label.text = model.deaInitiatordept;
            break;
        case 202:           //  制单日期
            label.text = [model.deaCreatetime componentsSeparatedByString:@"."].firstObject;
            break;
        case 203:           //  出差人
            label.text = model.deaEvectionpeople;
            break;
        case 204:           //  部门
            label.text = model.deaDept;
            break;
        case 205:           //  职务
            label.text = model.deaPost;
            break;
        case 206:           //  出差开始时间
            label.text = [model.deaBegintime componentsSeparatedByString:@" "].firstObject;
            break;
        case 207:           //  出差结束时间
            label.text = [model.deaEndtime componentsSeparatedByString:@" "].firstObject;
            break;
        case 208:           //  出差天数
            label.text = model.deaDay;
            break;
        case 209:           //  出差地点
            label.text = model.deaAddress;
            break;
        case 210:           //  出差事由
            label.text = model.deaReason;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromCCSPCell:_noteHeight + 10];
    }
}
