...
@implementation HTSPCell

- (void)creatHTSPApproveUIWithModel:(HTSPModel *)model
{
    _titleArr = @[@"发起人姓名",@"制单日期",@"合同名称",@"合同类别",@"合同金额(元)",
                  @"合同有效期",@"付款方式及进度",@"对方名称",@"联系方式"];
...
- (void)creatTextViewWithModel:(HTSPModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 200:           //  发起人姓名
            ...
            break;
        case 201:           //  制单日期
            label.text = [model.dcaCreatetime componentsSeparatedByString:@"."].firstObject;
            break;
        case 202:           //  合同名称
            label.text = model.dcaContractname;
            break;
        case 203:           //  合同类别
            label.text = model.dcaContracttype;
            break;
        case 204:           //  合同金额(元)
            label.text = [NSString stringWithFormat:@"%@",model.dcaMoney];
            break;
        case 205:           //  合同有效期
            label.text = model.dcaUsefullife;
            break;
        case 206:           // 付款方式及进度
            label.text = model.dcaPaytype;
            break;
        case 207:           //  对方名称
            label.text = model.dcaOppositename;
            break;
        case 208:           //  联系方式
            label.text = model.dcaOppositephone;
            break;
        default:
            break;
    }
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromHTSPCell:_noteHeight + 10];
    }
}
@end
