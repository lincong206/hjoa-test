...
{
    _titleArr = @[@"审理阶段",@"开庭日期",@"跟踪时间",@"案情描述",@"法律意见"];
...
- (void)creatTextViewWithModel:(FWGZModel *)model andLabel:(UILabel *)label andCount:(int)count andTitleLabel:(UILabel *)titleLabel andTitleSize:(CGSize )titleSize
{
    ...
    switch (label.tag) {
        case 100:
            label.text = model.ltfLnquisition;
            break;
        case 101:
            label.text = [model.ltfLawfutime componentsSeparatedByString:@" "].firstObject;
            break;
        case 102:
            label.text = [model.ltfTrackingtime componentsSeparatedByString:@" "].firstObject;
            break;
        case 103:
            label.text = model.ltfAccident;
            break;
        case 104:
            label.text = model.ltfLegaladvice;
            break;
        default:
            label.text = @"";
            break;
    ...
    if (count == _titleArr.count - 1) {
        [self.passHeightDelegate passHeightFromFWGZ:_noteHeight + 10];
    }
}
