...
// 旁边索引的排序
+(NSMutableArray*)IndexWithArray:(NSArray*)objectArray Key:(NSString *)key Sequence:(BOOL)sequence{
    ...
    // 将前面的#号放置最后
    if (sequence) {
        id A_Result_last = A_Result.firstObject;
        for (int i = 0; i < A_Result.count - 1; i ++) {
            A_Result[i] = A_Result[i + 1];
        }
        A_Result[A_Result.count - 1] = A_Result_last;
    }
    
    return A_Result;
}
//给对象数组排序
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key Sequence:(BOOL)sequence{
    //计算函数耗时
...
#pragma 取下表进行比较
    ...
    // 将前面的#号放置最后
    if (sequence) {
        id LetterResult_last = LetterResult.firstObject;
        for (int i = 0; i < LetterResult.count - 1; i ++) {
            LetterResult[i] = LetterResult[i + 1];
        }
        LetterResult[LetterResult.count - 1] = LetterResult_last;
    }
...
+(NSMutableArray*)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key{
    ...
    for(int i=0;i<[stringArr count];i++){
        ...
        if([chineseString.string isEqualToString:@" "] || [chineseString.string isEqualToString:@"深圳市华剑建设集团有限公司"] || [chineseString.string isEqualToString:@"(null)"]){
            chineseString.string = @"#";
        }
...
char pinyinFirstLetter(unsigned short hanzi)
{
    ...
    if (index >= 0 && index <= 20902){  //  即从第19968开始的20902个字符是中文简体字符
